...
$$ Y(p) = \frac{1}{1+\tau. p} X(p) \quad \underset{p.Y(p)\rightarrow \vec{\dot{y}} }{\longleftrightarrow} \quad \vec{y} + \tau \vec{\dot{y}}  = \vec{x}\quad \underset{\vec{x}=\vec{\delta_0}}{\longleftrightarrow} \quad \underbrace{h(t)}_{\vec{y}} = e^{-t/\tau}.\Gamma(t)$$

Ce système de pôle $\widehat{p_c}=\frac{-1}{\tau}$ est stable s.s.i. $\tau>
0$ car~:
...
trouve que le système est causal puisque la Réponse ImPulsionnelle (RIp) est
nulle avant la stimulation par l'impulsion~:
