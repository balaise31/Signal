...
$G$ peut être associé à sa réponse impulsionnelle $\vec{h}$ et
réciproquement tout signal $\vec{h}$ est associé au système dont il
...
impulsionnelle notée $\vec{h}$.

\texteimage{0.3}{op_h_to_sys}{0.6}{
  ...
  pour indiquer son rôle spécial pour le système $G$.
}
\imagetexte{0.4}{op_sys_to_h}{0.5}{
  Pour associer un système $G$ à un
  signal $h$, il suffit d'introduire une impulsion $\delta_0$ (de
  Dirac pour l'espace des signaux continus, et impulsion unité pour
  l'espace de signaux discret) en entrée du système $G$ pour
...
Au lieu de prendre le vecteur temporel $\delta_{0}$ pour identifier le
système à son signal de sortie, nous allons prendre plusieurs vecteurs
de la base fréquentielle $\vec{w_f}$ et associer un système à sa
réponse harmonique $G\p{f=\frac{\omega}{2\pi}}$ qui est aussi une
fonction de la variable réelle $\omega$ ou $f$. Dans le cas continu
l'onde pure est $\vec{\omega_f}:t\mapsto e^{i2\pi\,f\,t}$ qui devient
$\vec{\omega_f}:k\mapsto e^{i2\pi\,f\,k\,T_e}$ pour les systèmes
discrets.
...
\subsection{Algèbre des système et signaux}
Nous avons vu 
